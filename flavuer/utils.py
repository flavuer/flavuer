...
def create(args):
    ...
    os.system(f"python3 -m venv {args.name}/venv")  # noqa: S605

    # Install
    os.system(f"{args.name}/venv/bin/pip install Flask")  # noqa: S605

    print(f"Sucessfully created project {args.name}")
    print(Style.BRIGHT + f"Created flavuer project in {args.name}\n\n")

    print(Style.BRIGHT + "Get running for development")
    print(Fore.BLUE + f"\tcd {args.name}")
    print(Fore.BLUE + "\tflavuer dev")


def run_dev(args):
    print("Starting dev environment")
    os.system(  # noqa
        "export FLASK_DEBUG=1; export FLASK_APP=app; ./venv/bin/flask run"
    )  # noqa
